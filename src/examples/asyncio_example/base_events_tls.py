...
try:
    ...
    from tlslib.stdlib import OpenSSLClientContext, OpenSSLServerContext, OpenSSLTLSSocket
...
class BaseEventLoopTLS(events.AbstractEventLoop):
    def __init__(self):
    ...
    async def create_connection(
        ...
        *,
        ...
        """
        if server_hostname is not None and not tls:
            raise ValueError("server_hostname is only meaningful with tls")

        if server_hostname is None and tls:
        ...
        if tls_handshake_timeout is not None and not tls:
            raise ValueError("tls_handshake_timeout is only meaningful with tls")

        if tls_shutdown_timeout is not None and not tls:
            raise ValueError("tls_shutdown_timeout is only meaningful with tls")
        ...
        if self._debug:
            # Get the socket from the transport because TLS transport closes
            # the old socket and creates a new TLS socket
    ...
    async def _create_connection_transport(
        ...
        if tls:
            tlscontext = None if isinstance(tls, bool) else tls
    ...
    async def sendfile(self, transport, file, offset=0, count=None, *, fallback=True):
        ...
        (e.g. Windows or TLS socket on Unix).
    ...
    async def start_tls(
        ...
        *,
        ...
        """
        if tls is None:
            raise RuntimeError("Python tls module is not available")

        if not isinstance(tlscontext, OpenSSLClientContext | OpenSSLServerContext):
            raise TypeError(
                f"tlscontext is expected to be an instance of TLSClientContext"
                f"or TLSServerContext, got {tlscontext!r}"
            )
        ...
        # Pause early so that "tls_protocol.data_received()" doesn't
        # have a chance to get called before "tls_protocol.connection_made()".
        transport.pause_reading()

        transport.set_protocol(tls_protocol)
        conmade_cb = self.call_soon(tls_protocol.connection_made, transport)
    ...
    async def create_server(
        ...
        *,
        ...
        if isinstance(tls, bool):
            raise TypeError("tls argument must be an TLSClientContext or TLSServerContext or None")

        if tls_handshake_timeout is not None and tls is None:
            raise ValueError("tls_handshake_timeout is only meaningful with tls")

        if tls_shutdown_timeout is not None and tls is None:
            raise ValueError("tls_shutdown_timeout is only meaningful with tls")
