...
from . import selector_events_tls
...
def waitstatus_to_exitcode(status):
    """Convert a wait status to an exitcode."""
    try:
...
class _UnixSelectorEventLoop(selector_events_tls.BaseSelectorEventLoopTLS):
    """Unix event loop.
    ...
    def __init__(self, selector=None):
        """Initialize Unix event loop"""
        super().__init__(selector)
        self._signal_handlers = {}

    def close(self):
        """Close Unix event loop"""
        super().close()
        ...
        *,
        ...
        if tls:
            if server_hostname is None:
                raise ValueError("you have to pass server_hostname when using tls")
        else:
            ...
            if tls_shutdown_timeout is not None:
    ...
    async def create_unix_server(
        ...
        *,
        ...
        if isinstance(tls, bool):
            raise TypeError("tls argument must be an TLSContext or None")
...
class _UnixReadPipeTransport(transports.ReadTransport):
    ...
    def __repr__(self):
        ...
        if self._pipe is not None and selector is not None:
            polling = selector_events_tls._test_selector_event(
